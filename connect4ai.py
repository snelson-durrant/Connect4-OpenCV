...
while not game_over:
    ...
    # get player input
    print("your column selection")
    print(">", end=" ")
    col_select = int(input())

    # check player input and drop piece
    if 0 <= col_select <= COLUMN_COUNT - 1 and is_valid_location(
        game_board, col_select
    ):
        row = get_next_open_row(game_board, col_select)
        drop_piece(game_board, row, col_select, PLAYER_PIECE)
        ...
        # check for stalemate
        elif get_valid_locations(game_board) == []:
            print_board(game_board)
            print("STALEMATE!")
            game_over = True
            ...
            # get ai input and drop piece
            col_select = get_best_move(game_board, MINIMAX_DEPTH)[0]
            row = get_next_open_row(game_board, col_select)
            drop_piece(game_board, row, col_select, AI_PIECE)

            # check for ai win
            if four_in_a_row(game_board, AI_PIECE):
                print_board(game_board)
                print("AI WINS!")
                game_over = True
